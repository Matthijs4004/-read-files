...
with open("C:\Projecten\Software Dev\Mapje 9\-read-files\settings.yml","r") as file:
    prijs = yaml.safe_load(file)
...
aantalCaramel = 0
topping = 0
slagroomPrijs = prijs["toppings"]["slagroom"]
sprinkelsPrijs = prijs["toppings"]["sprinkels"]
caramelHoorntje = prijs["toppings"]["caramel"]["hoorentje"]
caramelBakje = prijs["toppings"]["caramel"]["bakje"]
bolletjePrijs = prijs["bolletjes"]
hoorntjePrijs = prijs["hoorentjes"]
bakjePrijs = prijs["bakjes"]
btw = prijs["btw"]
literPrijs = prijs["liter"]
...
def zakelijkeBon():
    liter = literInput.get()
    literP = liter * literPrijs
    quotient = literP / 100
    procent = quotient*btw
    destroyWidgets()
    bon1 = tk.Label(text="--------------[Papi Gelato]--------------\n",font=("Calibri Light", 12))
    bon2 = tk.Label(text=(f"Liter         {liter} x €{literPrijs} = €{round(literP, 3)}"),font=("Calibri Light", 12))
    bon3 = tk.Label(text='                                     ----- +',font=("Calibri Light", 12))
    bon4 = tk.Label(text=(f"Totaal                          €{round(literP,3)}"),font=("Calibri Light", 12))
    bon5 = tk.Label(text=(f"BTW ({btw}%)                     €{round(procent, 2)}"),font=("Calibri Light", 12))
...
def bon():
    ...
    if aantalBolletjes >= 1:
        bon2 = tk.Label(text=(f"Bolletje(s)   {aantalBolletjes} x €{bolletjePrijs} = €{round(totaalBol, 2)}"),font=("Calibri Light", 12))
        bon2.place(y=70,x=35)
    if aantalHoorntjes >= 1:
        bon3 = tk.Label(text=(f"Hoorntje(s)  {aantalHoorntjes} x €{hoorntjePrijs} = €{round(totaalHoorntje, 2)}"),font=("Calibri Light", 12))
        bon3.place(y=90,x=35)
    if aantalBakjes >= 1:
        bon4 = tk.Label(text=(f"Bakje(s)      {aantalBakjes} x €{bakjePrijs} = €{round(totaalBak, 2)}"),font=("Calibri Light", 12))
        bon4.place(y=110,x=35)
    if topping >= 1:
